  ...
  <table class="table table-striped">
			...
			<c:set var="num" value="${num }"></c:set>
				<c:forEach var="singoli" items="${singolist}">
                    ...
                    <c:when test="${singoli.category_no == 1}">
                        <a href="free_View.free?singo_board_no=${singoli.singo_board_no}" class="btn btn-success" target="_blank">자유게시판보기</a>
                    </c:when>
                    <c:when test="${singoli.category_no == 2}">
                        <a href="qna_View.qna?singo_board_no=${singoli.singo_board_no}" class="btn btn-warning" target="_blank">Q&A게시판보기</a>
                    </c:when>
                    <c:when test="${singoli.category_no == 3}">
                        <a href="study_View.study?singo_board_no=${singoli.singo_board_no}" class="btn btn-primary" target="_blank">스터디게시판보기</a>
                    </c:when>
                	</c:choose>				
					</td>
					<td>	<!-- 신고사유보기 -->
	...
	<!-- 검색 했을 경우의 페이징 처리 -->
			...
			<li><a href="singo_list.singo?pageNum=1&search=${search}&keyword=${keyword}">첫번째 페이지</a></li>
				<c:if test="${ap.currentPage > 1 }">
					<li><a
						href="singo_list.singo?pageNum=${ap.currentPage - 1}&search=${search}&keyword=${keyword}">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${ap.startPage}" end="${ap.endPage}">
					<li <c:if test="${ap.currentPage==i}">class="active"</c:if>><a
						href="singo_list.singo?pageNum=${i}&search=${search}&keyword=${keyword}">${i}</a></li>
				</c:forEach>
				<c:if test="${ap.currentPage < ap.totalPage}">
					<li><a
						href="singo_list.singo?pageNum=${ap.currentPage + 1}&search=${search}&keyword=${keyword}">다음</a></li>
				</c:if>
				<li><a href="singo_list.singo?pageNum=${ap.totalPage }&search=${search}&keyword=${keyword}">마지막 페이지</a></li>
			</c:if>
	...
	<!-- 전체 목록의 페이징 처리 -->
				<c:if test="${empty keyword}">
					...
					<c:if test="${ap.currentPage > 1 }">
						<li><a
							href="singo_list.singo?pageNum=${ap.currentPage - 1}">이전</a></li>
					</c:if>
					...
					<c:if test="${ap.currentPage < ap.totalPage }">
						<li><a
							href="singo_list.singo?pageNum=${ap.currentPage + 1}">다음</a></li>
					</c:if>
					<li><a href="singo_list.singo?pageNum=${ap.totalPage }">마지막
							페이지</a></li>
				</c:if>
	</ul>	 --%>			
